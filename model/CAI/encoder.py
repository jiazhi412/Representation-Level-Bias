...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.enc_e1 = nn.Linear(self.hidden_dim, h_dim)

        self.enc_cat = nn.Linear(h_dim+s_dim, h_dim)
    ...
    def forward(self, x, s):
        ...
        h = torch.cat([e1, s], dim=1)

        h = self.nz(self.enc_cat(h))
        return h
    ...
    #     super(Encoder, self).__init__()
    #     self.input_h, self.input_w, self.input_dep = input_shape
    #     self.hidden_dim = 6 * self.input_h // 2 * self.input_w // 2
    #
    #     self.enc_conv1 = nn.Conv2d(self.input_dep, 6, 5, stride=2, padding=2)
    #
    #     self.enc_s = nn.Linear(s_dim, hidden_dim)
    #
    # def forward(self, x, s):
    ...
    #
    #     self.enc_conv1 = nn.Conv2d(self.input_dep, 6, 5, stride=2, padding=2)
    #     self.enc_bn1 = nn.BatchNorm2d(6, momentum=0.01)
    #
    #     self.enc_conv2 = nn.Conv2d(6, 6, 5, stride=2, padding=2)
    #     self.enc_bn2 = nn.BatchNorm2d(6, momentum=0.01)
    #
    #     self.enc_conv3 = nn.Conv2d(6, 16, 5, stride=2, padding=2)
    #     self.enc_bn3 = nn.BatchNorm2d(16, momentum=0.01)
    #
    #     self.enc_e1 = nn.Linear(self.hidden_dim, hidden_dim)
    #     self.enc_e2 = nn.Linear(s_dim, hidden_dim)
    #
    #     self.enc_cat = nn.Linear(hidden_dim*2, h_dim)
    #
    #     if nz == 'tanh':
    #         self.nz = nn.Tanh()
    #     elif nz == 'sigmoid':
