...
from model.UAI_Adult.encoder import EncoderGerman
from model.UAI_Adult.reconstructor import ReconstructorGerman
from model.UAI_Adult.disentangler import DisentanglerGerman
from model.UAI_Adult.predictor import PredictorGerman
...
class UAI(nn.Module):
    ...
    def __init__(self, input_shape = 108, e1_dim = 40, e2_dim = 40, num_classes = 2):
        ...
        # ========= create models ===========
        self.encoder = EncoderGerman(input_shape=self.input_shape, e1_dim=self.e1_dim, e2_dim=self.e2_dim)
        self.reconstructor = ReconstructorGerman(drop_rate=0.5, output_shape=self.reconstruct_shape, e1_dim=self.e1_dim, e2_dim=self.e2_dim)
        self.predictor = PredictorGerman(e1_dim=self.e1_dim, num_classes=self.num_classes)
        self.disentangle_1_2 = DisentanglerGerman(in_dim=self.e1_dim, out_dim=self.e2_dim, actvation='tanh')
        self.disentangle_2_1 = DisentanglerGerman(in_dim=self.e2_dim, out_dim=self.e1_dim, actvation='tanh')
